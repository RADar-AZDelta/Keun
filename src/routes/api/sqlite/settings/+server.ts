...
async function checkForMissingValues(required: string[], settings: any) {
    let missing: string[] = []
    for (let req of required) if (!settings[req]) missing.push(req)
    if(missing.length > 0) return json({ result: 'error', message: 'The following properties were missing from the settings: ', missing, detail: ''})
}
