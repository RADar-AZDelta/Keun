...
export default class UsagiActions {
  ...
  private static async checkApprovalConditionsAndReturnUpdate() {
    ...
    const userIsTheLastAuthor = user === statusSetBy
    const conceptId = id ?? sourceAutoAssignedConceptIds
    if (userIsTheLastAuthor && (semiApproval || fullApproval)) return {}
    else if (semiApproval) return { 'ADD_INFO:approvedBy': user, 'ADD_INFO:approvedOn': Date.now(), mappingStatus: 'APPROVED' }
    else if (!semiApproval && !fullApproval) return { statusSetBy: user, statusSetOn: Date.now(), mappingStatus: 'SEMI-APPROVED', conceptId }
    else return {}
  }
  ...
  private static async checkFlaggingConditionsAndReturnUpdate() {
    ...
    return { statusSetBy: user, statusSetOn: Date.now(), mappingStatus: 'FLAGGED' }
  }
  ...
  private static async checkUnapprovalConditionsAndReturnUpdate() {
    ...
    return { statusSetBy: user, statusSetOn: Date.now(), mappingStatus: 'UNAPPROVED' }
  }
}
