export default class FileHelper {
  ...
  public static extractFilesFromInput = async (fileList: FileList | null, allowedExtensions: string[] | undefined) => {
    if (!fileList) return []
  ...
  private static extractSingleFile = async (file: File, allowedExtensions: string[] | undefined) => {
    const extension = file.name.split('.').pop()
    if (!extension) return undefined
    const extensionAllowed = allowedExtensions ? allowedExtensions.includes(extension) : true
