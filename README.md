...
If you want to run Keun locally you will need to provide credentials for Firebase. The cloud version of Keun requires Firebase credentials so you will need to update the ".env" file with your Firebase credentials.

- VITE_MAPPINGDATA_PATH = the path to get the concepts from
- VITE_ATHENA_DETAIL = the path to see the details about a certain concept
- PUBLIC_CLOUD_IMPLEMENTATION = this can be "none" or "firebase"

The other credentials can be found on Firebase if you create a new project.
...
Then you will need to create another ".env" file. For example ".env.local" where you put the following:

```bash
    SECRET_FIREBASE_ADMIN_SDK_CLIENT_EMAIL=
    SECRET_FIREBASE_ADMIN_SDK_PRIVATE_KEY=
